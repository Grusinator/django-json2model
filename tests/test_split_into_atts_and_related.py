...
class TestSplitIntoAttsAndRelated(TransactionTestCase):
    ...
    def test_split(self):
        data = self.build_test_data()

        atts, one2one_rel_objs, one2many_rel_objs = IJsonIterator()._split_into_attributes_and_related_objects(data)
        expected_atts = self.build_expected_attributes()
        expected_one2one_rel_objs = self.build_expected_one2one()
    ...
    def build_expected_one2one(self):
        return {
            'athlete': {'id': 465645, 'resource_state': 1},
            'map': {
                'id': 'a2880383294',
    ...
    def build_expected_attributes(self):
        return {
            'resource_state': 2, 'name': 'Morning Run', 'distance': 5618.2, 'moving_time': 1571,
            'elapsed_time': 1778, 'total_elevation_gain': 12.0, 'type': 'Run', 'id': 2880383294,
            'external_id': 'shealth12730eee-26b1-6297-9291-f488c87dd8ef.tcx', 'upload_id': 3058114478,
            'start_date': '2019-11-21T07:34:40Z', 'start_date_local': '2019-11-21T08:34:40Z',
            'timezone': '(GMT+01:00) Europe/Copenhagen', 'utc_offset': 3600.0,
            'start_latlng': [55.61765, 12.434259], 'end_latlng': [55.61774, 12.434354],
            'location_country': 'Denmark', 'start_latitude': 55.61765, 'start_longitude': 12.434259,
            'achievement_count': 0, 'kudos_count': 1, 'comment_count': 0, 'athlete_count': 1,
            'photo_count': 0, 'trainer': False, 'commute': False, 'manual': False, 'private': False,
            'visibility': 'everyone', 'flagged': False, 'from_accepted_tag': False,
            'upload_id_str': '3058114478', 'average_speed': 3.576, 'max_speed': 5.9,
            'average_cadence': 84.8, 'has_heartrate': True, 'average_heartrate': 109.3,
            'max_heartrate': 142.0, 'heartrate_opt_out': False, 'display_hide_heartrate_option': True,
            'elev_high': 3.0, 'elev_low': -2.5, 'pr_count': 0, 'total_photo_count': 0, 'has_kudoed': False
        }
